...
export default async function handler(req, res) {
  ...
  try {
    ...
    const [totalProducts, totalProposals, totalReports, recentReports] = await Promise.all([
      ...
      ImpactReport.find().sort({ createdAt: -1 }).limit(5).select("order_id plastic_saved_kg carbon_avoided_kg createdAt"),
    ]);
    ...
    const aggregated = await ImpactReport.aggregate([
      {
        $group: {
          _id: null,
          total_plastic_kg: { $sum: "$plastic_saved_kg" },
          total_carbon_kg: { $sum: "$carbon_avoided_kg" },
        },
      },
    ]);
